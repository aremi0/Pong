...
void DrawLogoScreen(void)
{
    ...
    else if (state == 3)    // Draw "raylib" text-write animation + "powered by"
    {
        ...
        DrawText(TextSubtext("aremi", 0, lettersCount), GetScreenWidth()/2 - 44, GetScreenHeight()/2 + 48, 50, Fade(BLACK, alpha));

        if (framesCounter > 20) DrawText("developed by", logoPositionX, logoPositionY - 27, 20, Fade(DARKGRAY, alpha));
    }
}
